...
from transforms3d.quaternions import axangle2quat, mat2quat, qmult, qinverse, quat2axangle
...
class YCBRenderer:
    def __init__(self, width=512, height=512, render_marker=False, gpu_id=0, model_scale=1.0):
        ...
        self.r = CppYCBRenderer.CppYCBRenderer(width, height, gpu_id)
        self.r.init()
    ...
    def set_model_scale(self, model_scale):
        self.model_scale = model_scale
    ...
    def set_projection_matrix(self, w, h, fu, fv, u0, v0, znear, zfar):
        znear /= self.model_scale
        zfar /= self.model_scale
        L = -(u0) * znear / fu;
        R = +(w - u0) * znear / fu;
        T = -(v0) * znear / fv;
        B = +(h - v0) * znear / fv;
    ...
    def render(self, cls_indexes, image_tensor=None, seg_tensor=None, pc1_tensor=None, pc2_tensor=None, cpu_only=False):
        frame = 0
    ...
    def set_poses(self, poses):
        ...
        self.poses_trans = [np.ascontiguousarray(
            xyz2mat(item[:3] / self.model_scale)) for item in poses]
    ...
    def clean(self):
        GL.glDeleteTextures([self.color_tex, self.color_tex_2,
                             self.color_tex_3, self.color_tex_4,
                             self.color_tex_5, self.depth_tex])
        self.color_tex = None
        self.color_tex_2 = None
        self.color_tex_3 = None
        self.color_tex_4 = None
        self.color_tex_5 = None
...
if __name__ == '__main__':
    ...
    model_path = os.path.join(curr_path, '../data/curiosity/models/block_23/')
    ...
    image_path = '../data/real_camera_A/rgb_006.png'
