...
def run(
        ...
        abs_gpu_id=None,
        ref_checkpoint=None,
        det_checkpoint=None, cfg_file=None, class_name_det=None, class_name_ref=None, class_name_test=None,
        trainset_name=None, testset_name=None,
        phase=None, tracking=None, randomize=None, vis=None, no_log=None, no_cache=None, trainer=None
):
    is_array = False
    ...
    if data_mode == 'images':
        ...
        pose_gts = glob.glob('/home/yxh/文档/ObjectDatasetTools-master/LINEMOD/arqc/pose_gts/*.npy')
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        if det.size(0) == 0:
            ...
            if data_mode == 'images':
                init_queue.put(
                    {'im': output, 'cls_box': [int(-1), int(0), int(0), int(640), int(640)], 'gts': gts[runcnt]})
            else:
                init_queue.put({'im': output, 'cls_box': [int(-1), int(0), int(0), int(640), int(640)], 'gts': None})
        # cv2.imwrite('yolosee.png', output.astype(np.uint8))
        else:
            # print('定位中...')
            res = det[:, :].cpu().numpy()[0]
            cls = res[5]
            ...
            # output = cv2.rectangle(output, (int(box[0]), int(box[1]) + 80), (int(box[2]), int(box[3]) + 80),
            #                        (0, 0, 255), 3)
            # output = cv2.putText(output, 'view class:' + str(cls), (int(box[0]), int(box[1]) + 80),
            #                      cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 2)
            # cv2.imshow('res', output.astype(np.uint8))
            # cv2.imwrite('./yolo_cls_' + str(cls) + '.png', output.astype(np.uint8))
            # cv2.imshow('res', output.astype(np.uint8))

            init_queue.queue.clear()
            if data_mode == 'images':
                init_queue.put({'im': output, 'cls_box': [int(cls), int(box[0]), int(box[1] + 80), int(box[2] - box[0]),
                                                          int(box[3] - box[1])], 'gts': gts[runcnt]})
            else:
                init_queue.put({'im': output, 'cls_box': [int(cls), int(box[0]), int(box[1] + 80), int(box[2] - box[0]),
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str,
                        ...
                        # default='/home/yxh/文档/yolov5/runs/train/AR_QC/weights/best.pt',
...
def main(opt, init_queue, track_queue, data_mode):
    # check_requirements(ROOT / 'requirements.txt', exclude=('tensorboard', 'thop'))
    # opt.source = cv2.imread(r'/home/yxh/文档/ObjectDatasetTools-master/LINEMOD/arqc/JPEGImages/0.jpg')
    import glob
    if data_mode == 'images':
        opt.source = '/home/yxh/文档/ObjectDatasetTools-master/LINEMOD/arqc/JPEGImages/'

    return run(init_queue, track_queue, data_mode, **vars(opt))


def pose_init_v5(init_queue, track_queue, data_mode):
    return main(opt, init_queue, track_queue, data_mode)
