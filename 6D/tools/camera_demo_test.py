#!/home/yxh/anaconda3/envs/honda/bin/python
...
# 预先定义防止opengl驱动报错
renderer = YCBRenderer(width=640,
                       height=640,
                       gpu_id=0,
                       render_marker=False,
                       model_scale=1.0)
...
from yolov5.detect_camera import pose_init_v5
from my_task import pose_init_v8
import os

init_queue = LifoQueue()
track_queue = LifoQueue()
init_file = open('./tools/init_config.json', 'r')
init_dict = json.load(init_file)
STANDARD_POSES = init_dict['STANDARD_POSES']
AREA = init_dict['AREA']
init_file.close()
poses_pool = {'init': None, 'last': None}
K_DeepIM = np.array([[524.7917885754071, 0, 332.5213232846151],
                     [0, 489.3563960810721, 281.2339855172282],
                     [0, 0, 1]])
# K_camera = np.array([[606.19, 0, 324.23], [0, 606.17, 250.51], [0, 0, 1]])
K_camera = np.array([[606.19, 0, 324.23], [0, 606.17, 324.23], [0, 0, 1]])
...
acc_5d5c = AverageMeter()
times_test = AverageMeter()
...
class SimpleTester(object):
    ...
    def test_one_image(self, data, prefix=0):
        ...
        if cfg.TEST.VISUALIZE or ('gt_pose' in data):
            ...
            if cfg.TEST.VISUALIZE:
                ...
                for i in range(len(data_ref)):
                    ...
                    cv2.imwrite(os.path.join(self.vis_dir, vis_fname), (vis[:, :, (2, 1, 0)] * 255).astype('uint8'))
                    vis, render_box = self.visualize(data_ref[i], 'rend')
            ...
            if 'gt_pose' in data:
                ...
                print("re {:.3f}->{:.3f} te {:.3f}->{:.3f}, {:.3f} seconds".format(re_init, re_ours, te_init, te_ours,
                                                                                   t_use))

                if re_ours < 5. and te_ours * depth_scale < 5.:
                    acc_5d5c.update(1)
                ...
                print(acc_5d5c.avg)
                if re_ours > 10. or te_ours * depth_scale > 10.:
                    init_flag = True

            # if len(correct_list) == 1200:
            #     with open(r"./correct_list_official2.txt", 'w')as f:
        ...
        if num_iter_refine == 0:
            pose_est_mat = data['init_pose']
        else:
            pose_est_mat = preds['poses_pred'][0].cpu().numpy()
            quat = copy.deepcopy(preds['quat'])
            trans = copy.deepcopy(preds['trans'])

        im_pose = (vis * 255).astype(np.uint8)
        pose = data_ref[-1]['pose_rend'].detach().cpu().numpy()
        im_pose = np.ascontiguousarray(im_pose)
        times_test.update(t_use)
        # cv2.putText(im_pose, 'fps:' + str(1 / times_test.avg)[:5], (20, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.75,
        #             (255, 0, 0), 1)
        cv2.putText(im_pose, 'acc_5d5cm:' + str(acc_5d5c.avg)[:5], (20, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.75,
                    (255, 0, 0), 1)

        return im_pose, pose, render_box, init_flag
    ...
    def visualize(self, data, type='rend'):
        ...
        object_pred = quick_rend(pose_rend)

        # get render box
        binary1 = data['mask_rend'].detach().cpu().numpy()
        contour1, hierarchy = cv2.findContours(binary1.astype(np.uint8), cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        try:
            contour1 = sorted(contour1, key=cv2.contourArea)
            x, y, w, h = cv2.boundingRect(contour1[-1])
        except:
            x, y, w, h = 0, 0, 640, 640

        render_box = [x, y, w, h]

        combined = image_obs * 0.4 + object_pred[:, :, (2, 1, 0)] * 0.6
        if type == 'rend':
            return combined[:, :, [2, 1, 0]], render_box
        else:
            return combined[:, :, [2, 1, 0]]
...
def load_network(num_classes):
    ...
    network = networks.__dict__['dcm_ot_v0'](num_classes, network_data, inference=True).cuda()
    return network
...
def get_init(cls_box, data):
    # poses_input格式:[(z, y, x, w), 平移向量]
    init_result = cls_box
    poses_input = np.zeros((1, 7), dtype=np.float32)

    if cls_box[0] != -1:
        # rot
        poses_input[:, 0:4] = np.array(STANDARD_POSES[str(init_result[0])])
        # trans
        xl, yt, w, h = init_result[1:]
        S_test = w * h
        tz = math.sqrt(AREA[data['class_idx'] - 1][str(init_result[0])] / S_test)
        tz = tz + 0.2
        u_test, v_test = xl + w / 2, yt + h / 2

        tx, ty = np.dot(np.linalg.inv(K_camera), np.array([[u_test], [v_test], [1.00]]))[(0, 1), 0] * tz
        poses_input[:, 4:] = np.array([tx, ty, tz])
        data['init_pose'] = poses_input
        print('初始化成功')

        # qsc = (data['image'] * 255).astype(np.uint8)
        # cv2.rectangle(qsc, (cls_box[1], cls_box[2]), (cls_box[3] + cls_box[1], cls_box[4] + cls_box[2]),
        #               (255, 0, 0), 1)
        # cv2.imwrite('./3.png', qsc)

    else:
        if poses_pool['last'] is not None:
            data['init_pose'] = poses_pool['last']
            print('初始化失败, 选择上一轮初始结果:{}作为初始位姿'.format(data['init_pose']))
        else:
            if 'gt_pose' not in data:
                data['init_pose'] = np.array([[0, 1, 0, 0, 0, 0, 0.8]])
            else:
                data['init_pose'] = np.array([data['gt_pose']])

                # from transforms3d.euler import euler2quat, quat2euler
                # euler = quat2euler(data['init_pose'][0, :4])
                # euler += 15 * np.random.randn(3) * pi / 180.0
                # data['init_pose'][0, :4] = euler2quat(euler[0], euler[1], euler[2])
                # std_trans = [0.01, 0.01, 0.05]
                # data['init_pose'][0, 4] = data['init_pose'][0, 4] + std_trans[0] * np.random.randn()
                # data['init_pose'][0, 5] = data['init_pose'][0, 5] + std_trans[1] * np.random.randn()
                # data['init_pose'][0, 6] = data['init_pose'][0, 6] + std_trans[2] * np.random.randn()

                print('初始化失败, 选择预设位姿:{}作为初始位姿'.format(data['init_pose']))
...
def inference(infer_cfg, class_names):
    data = {}
    init_result = init_queue.get()
    im, cls_box = init_result['im'], init_result['cls_box']
    data['K'] = K_camera
    data['class_idx'] = class_names.index(infer_cfg["track_target"])
    data['init_pose'] = None
    data['image'] = im / 255
    data['depth'] = im[:, :, 0]

    if init_result['gts'] is not None:
        data['gt_pose'] = init_result['gts']

    if poses_pool['init'] is None:
        # 第一次或者丢失目标后重新初始化
        print('初始化...')
        data = get_init(cls_box, data)
    else:
        print('继续跟踪...')
        data['init_pose'] = poses_pool['init']

    # 跟踪优化
    with torch.no_grad():
        im_pose_color, pose_result, render_box, init_flag = train.test_one_image(data, 0)

    # 计算结果交并比
    iou = get_iou([cls_box[1], cls_box[2], cls_box[1] + cls_box[3], cls_box[2] + cls_box[4]],
                  [render_box[0], render_box[1], render_box[0] + render_box[2], render_box[1] + render_box[3]])

    # if iou < 0.3:
    if (init_result['gts'] is not None and init_flag) or (init_result['gts'] is None and iou < 0.3):
        poses_pool['last'] = poses_pool['init']
        poses_pool['init'] = None
        print('目标跟丢')
    else:
        poses_pool['init'] = pose_result

        print('目标跟踪正常')

    cv2.putText(im_pose_color, 'box_iou:' + str(iou)[:5], (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 0, 0), 1)
    cv2.rectangle(im_pose_color, (cls_box[1], cls_box[2]), (cls_box[3] + cls_box[1], cls_box[4] + cls_box[2]),
                  (255, 0, 0), 1)
    track_queue.put(im_pose_color[:, :, (2, 1, 0)])
...
if __name__ == '__main__':
    ...
    infer_file = open('tools/infer_cfg.json', 'r')
    infer_cfg = json.load(infer_file)
    infer_file.close()

    class_names = ['__background__', 'arqc', 'squirrel', 'can', 'ape', 'cat', 'duck', 'iron']
    ...
    # yolo位姿初始化
    init_thread = threading.Thread(target=pose_init_v8, args=(init_queue, track_queue, infer_cfg,))
    # init_thread = mp.Process(target=pose_init, args=(init_queue, ))
    init_thread.start()

    # DeepIM位姿细化
    # refine_thread = threading.Thread(target=inference)
    # refine_thread = mp.Process(target=inference)
    # refine_thread.start()
    t = 0
    while True:
        print('#########################################')
        print('loop-{} start:'.format(t))
        inference(infer_cfg, class_names)
        print('loop-{} end'.format(t))
        t += 1
