...
class DeepCodeMatchingTraining(object):
	...
	def build_tf_log(self):
		if self.at_train():
			...
			log_dir = os.path.join(train_output_dir, 'tf_log')
	...
	def sample_poses_refine(self, instance_target):
		...
		if init_pose_type == 'fixed':
			euler = self.cfg.TRAIN.REF.INIT_FIXED_POSE if (
						self.at_train() or self.at_val()) else self.cfg.TEST.REF.INIT_FIXED_POSE
	...
	def update_data_refine(self, data_ref, targets_ref, preds):
		...
		for sample_idx in range(num_samples):
			...
			# import copy
			# rgb = copy.deepcopy(image_obs[:1, :, :, :]).cpu().squeeze(0).permute(1, 2, 0).numpy()
			# rgb = cv2.normalize(rgb, None, 0, 255, norm_type=cv2.NORM_MINMAX)
			# rend = copy.deepcopy(image_rend[:1, :, :, :]).cpu().squeeze(0).permute(1, 2, 0).numpy()
			# rend = cv2.normalize(rend, None, 0, 255, norm_type=cv2.NORM_MINMAX)
			# cv2.imwrite('./train_samples'+str(sample_idx)+'.png', np.hstack((rgb.astype(np.uint8), rend.astype(np.uint8))))
	...
	def loss_angular_distance(self, quat_est, quat_gt):
		errors_rot = torch.clamp(
			torch.acos(torch.clamp(2 * (torch.sum(quat_est * quat_gt, axis=1) ** 2) - 1, min=0, max=1)) - 0.05, min=0.)
		return errors_rot
	...
	def track_refine(self):
		...
		for i_batch, (data_cuda, targets) in enumerate(tqdm(self.test_loader)):
			...
			class_name = self._dataset_test._class_names_used[data_ref[i]['class_idx']]
			vis_dir = os.path.join(self.cfg.TRAIN.REF.OUTPUT_DIR,
			                       '../track-{}-{}'.format(class_name, video_id_cur_frame))
			if not os.path.exists(vis_dir):
				os.mkdir(vis_dir)
