...
class Curiosity_Base(data.Dataset, imdb):
    def __init__(self, image_set, dataset_root_path, mode, class_names=None):
        ...
        self._class_colors_all = [(x * self.cfg.DATA.DISCRET_COLOR_STEP,) * 3 for x in
                                  range(len(self._class_names_all))]
        self._symmetry_all = np.array([0] * self._num_classes_all)
        self._sym_info_all = np.ones(
            [len(self._class_names_all), 10, 4]) * -1  # use 10 as the max number of gt candidates
    ...
    def setup_synthetic(self):
        ...
        self._valid_range = np.array(
            [[margin, self._width - margin], [margin, self._height - margin], [self.cfg.SYN.TNEAR, self.cfg.SYN.TFAR]])
    ...
    def process_targets(self, masks, fg_class_ids, fg_poses):
        ...
        complete_boxes = []
        pixels = pts2pixels(self.get_point_clouds('std')[class_idx[0]], quat2mat(fg_poses[0, :4]), fg_poses[0, 4:],
    ...
    def _evaluate(self, ind, pose_est, pose_tgt, intrinsic_matrix, class_idxs):
        ...
        for i in range(pose_est.shape[0]):
            ...
            # reprojection error
            error_reprojection = reproj(intrinsic_matrix[i], RT_est[:3, :3], RT_est[:, 3], RT_tgt[:3, :3], RT_tgt[:, 3],
                                        self._points_std[class_idx])
    ...
    def print_pose_accuracy(self, mute=False):
        for cls_ind, abs_ind in enumerate(self.class_ids):
            ...
            for i in range(cfg.TEST.REF.ITERNUM + 2):
                ...
                if not mute:
                    print_and_log('5 degree, 5cm accuracy:   %.2f (%.2f, %.2f)' % (
                    self.acc[cls_ind, i, 0] * 100., self.acc[cls_ind, i, 8] * 100., self.acc[cls_ind, i, 9] * 100.))
            ...
            if len(self.class_ids) >= 1:
                ...
                for i in [cfg.TEST.REF.ITERNUM]:
                    ...
                    for metric_id in range(6):
                        metric_list = ['5d 5cm', '6D Pose ADD <0.2cm', '6D Pose ADI <0.2cm', 'ReProj 2D Acc',
                                       'VOCap ADD', 'VOCap ADI']
                        ...
                        for cls_ind, abs_ind in enumerate(self.class_ids):
                            if abs_ind == 0:
                                continue
                            print_and_log('{: <25} {:.2f}'.format(self._class_names_all[abs_ind],
                                                                  self.acc[cls_ind, i, metric_id] * 100.))
