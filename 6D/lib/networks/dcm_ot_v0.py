...
import cv2
import numpy as np
from lib.DBB.diversebranchblock import DiverseBranchBlock as dbb
...
class RepFlowNetS(nn.Module):
    def __init__(self, batchNorm=False, has_deconv=False, input_channel_size=0, expansion=64, deploy=False):
        super(RepFlowNetS, self).__init__()

        self.has_deconv = has_deconv
        self.batchNorm = batchNorm
        self.act = nn.LeakyReLU(0.1, True)
        self.conv1 = dbb(input_channel_size, expansion * 1, 7, 2, 3, batch_Norm=batchNorm, deploy=deploy,
                         nonlinear=self.act)
        self.conv2 = dbb(expansion * 1, expansion * 2, 5, 2, 2, batch_Norm=batchNorm, deploy=deploy, nonlinear=self.act)
        self.conv3 = dbb(expansion * 2, expansion * 4, 5, 2, 2, batch_Norm=batchNorm, deploy=deploy, nonlinear=self.act)
        self.conv3_1 = dbb(expansion * 4, expansion * 4, 3, padding=1, batch_Norm=batchNorm, deploy=deploy,
                           nonlinear=self.act)
        self.conv4 = dbb(expansion * 4, expansion * 8, 3, 2, 1, batch_Norm=batchNorm, deploy=deploy, nonlinear=self.act)
        self.conv4_1 = dbb(expansion * 8, expansion * 8, 3, padding=1, batch_Norm=batchNorm, deploy=deploy,
                           nonlinear=self.act)
        self.conv5 = dbb(expansion * 8, expansion * 8, 3, 2, 1, batch_Norm=batchNorm, deploy=deploy, nonlinear=self.act)
        self.conv5_1 = dbb(expansion * 8, expansion * 8, 3, padding=1, batch_Norm=batchNorm, deploy=deploy,
                           nonlinear=self.act)
        self.conv6 = dbb(expansion * 8, expansion * 16, 3, 2, 1, batch_Norm=batchNorm, deploy=deploy,
                         nonlinear=self.act)
        self.conv6_1 = dbb(expansion * 16, expansion * 16, 3, padding=1, batch_Norm=batchNorm, deploy=deploy,
                           nonlinear=self.act)

        if has_deconv:
            self.deconv5 = deconv(expansion * 16, expansion * 8)
            self.deconv4 = deconv(expansion * 16 + 2, expansion * 4)
            self.deconv3 = deconv(expansion * 8 + expansion * 4 + 2, expansion * 2)
            self.deconv2 = deconv(expansion * 4 + expansion * 2 + 2, expansion * 1)

            self.predict_flow6 = predict_flow(expansion * 16)
            self.predict_flow5 = predict_flow(expansion * 16 + 2)
            self.predict_flow4 = predict_flow(expansion * 8 + expansion * 4 + 2)
            self.predict_flow3 = predict_flow(expansion * 4 + expansion * 2 + 2)
            self.predict_flow2 = predict_flow(expansion * 2 + expansion * 1 + 2)
    ...
    def rep(self):
        for m in [self.conv1, self.conv2, self.conv3, self.conv3_1, self.conv4,
                  self.conv4_1, self.conv5, self.conv5_1, self.conv6, self.conv6_1]:
            try:
                m.switch_to_deploy()
                print('succeed to reparameter with faster speed!!! ')
            except:
                print('failed to reparameter, may be a liitle slow!!!')
                pass
...
class DCMOT(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        if self.network_cfg.BASENET_TYPE == 'flownets':
            ...
            # self._fcn = RepFlowNetS(has_deconv=self.has_deconv, expansion=48, input_channel_size=input_channel_size)
            # out_channels = 48*16
            # print(self._fcn)
        ...
        if self.network_cfg.BASENET_TYPE == 'flownets':
            ...
            if os.path.exists(official_model):
                pretrained_dict = torch.load('../data/checkpoints/flownets_EPE1.951.pth.tar',
    ...
    def forward(self, **kwargs):
        ...
        # import copy
        # for kkk in range(rgb_obs.size(0)):
        #     rgb = copy.deepcopy(rgb_obs[kkk:kkk+1, :, :, :]).cpu().squeeze(0).permute(1, 2, 0).numpy()
        #     rgb = cv2.normalize(rgb, None, 0, 255, norm_type=cv2.NORM_MINMAX)
        #     rend = copy.deepcopy(rgb_rend[kkk:kkk+1, :, :, :]).cpu().squeeze(0).permute(1, 2, 0).numpy()
        #     rend = cv2.normalize(rend, None, 0, 255, norm_type=cv2.NORM_MINMAX)
        #     # cv2.imwrite('./train_samples_zoom'+str(kkk)+'.png', np.hstack((rgb.astype(np.uint8), rend.astype(np.uint8))))
        #
        # cv2.imshow('rgb', np.hstack((rgb,rend)).astype(np.uint8))
        # cv2.waitKey(1)
...
def dcm_ot_v0(num_classes, pretrained_dict=None, inference=False):
    ...
    # 网络结构重参数化
    if inference and hasattr(model._fcn, 'rep'):
        model._fcn.rep()
